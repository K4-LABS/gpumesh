...
(`DEFAULT_BIND_HOST`, `cli.py:341`; `_resolve_bind_host`, `cli.py:356`;
`GPUMesh.start_coordinator(host="127.0.0.1")`, `api.py:103`). Everything below
...
device that submitted code will run as (`cli.py:406`, called at `cli.py:518`;
the API equivalent at `api.py:167-179`).

Two listeners are still wide by design, and both are noted where they appear
below: the worker claim server (`claimer.py:585`) and `gpumesh setup` in LAN
...
COORDINATOR
  5. _authed()  -> SecurityManager.verify_request   server.py:339, server.py:276
                                                    security.py:239
  6. safe-mode gate: 403 if script contains __gpumesh_function__   server.py:474
  7. db.create_job -> payload stored as JSON text in SQLite  db.py:487, db.py:507

WORKER
  8. POST /api/lease  -> task JSON                  server.py:445
  9. safe-mode gate: refuse function tasks          worker.py:832
 10. serializer.deserialize_function(payload["_func"])   worker.py:145
       -> cloudpickle.loads(rest)                   serializer.py:217
       -> OR exec(compile(source, ...), namespace)  serializer.py:265
          (cross-version fallback)
       -> OR refuse, user_error=True                worker.py:160
 11. cloudpickle.dumps((func, params)) -> subprocess stdin   worker.py:180, worker.py:201
 ...
 15. POST /api/result  with the envelope            worker.py:846

COORDINATOR
 16. db.complete_task stores result as JSON text    db.py:845, db.py:861

CLIENT
 17. GET /api/jobs/<id>                             server.py:321
 18. serializer.decode_result(raw)                  serializer.py:498
       -> cloudpickle.loads(base64decode(value))    serializer.py:520
     call sites: api.py:450, accelerate.py:383, client.py:37
...
budget (`db.py:845`), so a deterministically broken task burns one attempt
...
| **The token** | `--token` argv (on `serve` and every other subcommand's parser), stdout at `cli.py:493`, `GPUMESH_TOKEN` env, `~/.gpumesh/config.json` at 0600 (`connection_manager.py:23`, `connection_manager.py:101`), and the coordinator's memory as a hash (`hash_token`, `security.py:38`) | It is the single credential. Holding it is equivalent to being the operator. |
| **Code in flight** | The cloudpickle blob in `payload["_func"]`, stored as JSON text in SQLite (`db.py:507`) and moving over plain HTTP | Reading it discloses proprietary model code; modifying it is remote code execution on every worker. |
| **Data in flight and results** | Payloads and result envelopes, plain HTTP, stored in SQLite (`db.py:861`) | Training data, inference outputs. Also the injection vector for step 18. |
...
`cmd_serve` resolves the bind through `_resolve_bind_host` (`cli.py:356`) —
`--host`, then `GPUMESH_HOST`, then `DEFAULT_BIND_HOST = "127.0.0.1"`
(`cli.py:341`) — and passes it to `server.serve` (`cli.py:473`), where the
socket is bound (`server.py:590`). `GPUMesh.start_coordinator` mirrors the
default in its signature (`api.py:103`) and calls `serve` by keyword
specifically so a future parameter reorder cannot silently bind the wrong
argument (`api.py:141`).

Every route is gated by `_authed()` (`server.py:273`), which runs **before**
the body is read on both verbs (`server.py:292` for GET, `server.py:339` for
POST, with `_read_json` at `server.py:217`). Bodies are capped at 10 MB
(`server.py:121`) and oversized ones are drained rather than left to desync the
connection (`server.py:179-214`).
...
- **Exposure is opt-in and loud.** A non-loopback bind prints
  `_print_exposure_warning` (`cli.py:406`, called at `cli.py:518`), naming the
  OS user and device. A loopback bind deliberately refuses to print a LAN join
  URL, and prints the exact command to open up instead (`cli.py:499-511`), so
  ...
  (`cli.py:466`).
- **Consistency check:** `_is_loopback_bind` (`cli.py:344`) treats an *empty*
  host as non-loopback, because `bind(("", port))` is `INADDR_ANY`. The saved
  connection URL follows the bind (`cli.py:489-491`), so a loopback
...
- **Caveat — `gpumesh setup`.** The wizard's LAN auto-discovery mode overrides
  ...
  (`setup_wizard.py:466-475`), bound at `setup_wizard.py:577`. This is
  ...
  `setup_wizard.py:611`). If `GPUMESH_HOST` pins loopback, the wizard says so
  and explains why the flow will find nothing (`setup_wizard.py:482-495`).
...
`claimer.start_claim_server` (`claimer.py:535`) resolves its bind address
through `_resolve_claim_bind_host` (`claimer.py:40`) and binds it at
`claimer.py:585`. The precedence is deliberately the same shape as the
...
(`claimer.py:73`, called at `claimer.py:597`) prints a banner naming the
...
(`claimer.py:478`). `gpumesh worker` monkey-patches `ClaimHandler.do_POST`
with its own `_intercepted_do_POST` (`worker.py:994`, installed at
`worker.py:1074`); that patched handler calls the *same* `_read_json` and
`_send`, and repeats the same `hmac.compare_digest` (`worker.py:1029`), so
...
- **Pre-auth attack surface is structurally larger here than on the
  ...
  - **Rate limit first.** `_read_json` (`claimer.py:340`) consults
    `RateLimiter.is_allowed` before it reads `Content-Length` at all
    (`claimer.py:356`), answering 429 and closing. A locked-out peer therefore
    cannot drive the JSON parser.
  - **16 KB body cap, refused on the declared length.** `MAX_CONTENT_LENGTH`
    is `16 * 1024` (`claimer.py:227`) and an oversized declaration is rejected
    at `claimer.py:407` **before any allocation proportional to it**. The
    ...
    is itself bounded at 256 KB with a 2 s timeout (`claimer.py:236-237`,
    enforced in `_drain` at `claimer.py:305-336`), so
  ...
  - **No chunked, no length-less requests.** `BaseHTTPRequestHandler` does not
    ...
    (`claimer.py:381`, `claimer.py:389`) rather than half-understood.
  - **15 s socket timeout** (`timeout = 15`, `claimer.py:246`). This closes a
  ...
  - **Rate-limit accounting is centralised in `_send`** (`claimer.py:267`,
    accounting at `claimer.py:287-289`), the one method both `do_POST`
...
- **The limiter is `security.RateLimiter`, not a second implementation**
  (imported `claimer.py:22`, instantiated `claimer.py:217`) — same defaults
  ...
  (`claimer.py:582`), on the reasoning that a restart is a deliberate act at
...
- On success the worker calls out to a coordinator URL taken from the request
  body (`find_reachable_coordinator`, called at `claimer.py:504`;
  `worker.py:1054` in the patched handler) and only then joins it
  (`claimer.py:522`; `worker.py:1071` acknowledges). That is an outbound fetch
...
nearby peers (`_discovery_printer`, `server.py:553`).
...
- **Spoofing:** trivial. Anyone can broadcast a beacon claiming to be a
  ...
  (`server.py:347`), never auto-claimed, and field values are length- and
...
| --- | --- | --- | --- |
| Coordinator -> worker | `serializer.py:217` (`cloudpickle.loads`), `serializer.py:265` (`exec` of source), reached from `worker.py:145` | The coordinator, and therefore every token holder | Token, plus `--safe-mode` (`worker.py:832`, `server.py:474`) |
| Worker -> worker subprocess | `_function_subprocess.py:65` | The worker process itself | None needed; same trust |
| **Worker -> client** | **`serializer.py:520`** (inside `decode_result`, `serializer.py:498`), reached from `api.py:450`, `accelerate.py:383`, `client.py:37` | **Every worker in the mesh** | **Token only. `--safe-mode` does not cover this.** |
...
(`connection_manager.py:77-81`), chmod'd 0600 (`connection_manager.py:101`),
and on Windows given a best-effort `icacls /inheritance:r`
(`connection_manager.py:119-121`).

- **Boundary:** OS file permissions.
- **Failure is no longer silent.** Both the chmod and the `icacls` call route
  their failures to `_warn_permissions` (`connection_manager.py:44`), which
  prints the file path, what specifically failed, and what the token grants.
  Four distinct failures are surfaced: chmod raising
  (`connection_manager.py:102-103`), `USERNAME` unset so `icacls` cannot be
  targeted (`connection_manager.py:110-114`), `icacls` exiting non-zero with
  its own stderr quoted (`connection_manager.py:122-127`), and `icacls` not
  being runnable at all (`connection_manager.py:128-129`).
- **Permissions are also re-checked on read.** `load_connection` calls
  `_warn_if_world_readable` (`connection_manager.py:135`, called at
  `connection_manager.py:182`), which warns if `st_mode & 0o077` is set. This
  ...
  (`connection_manager.py:132`), so the warning is not tuned out by repetition.
- **POSIX only, deliberately.** The read-time check returns early on Windows
  (`connection_manager.py:156`) because `st_mode` there carries no real ACL
...
- **Not covered:** a failed write is still fatal and re-raised
  (`connection_manager.py:82-87`), which is correct — a partial config is
  worse than none.
...
| --- | --- | --- | --- | --- | --- |
| 0 | Coordinator reachable from the LAN at all | Elevation (precondition for 1, 2, 4, 5, 10) | Any remote TCP connection to port 8000 | **Loopback bind by default** (`cli.py:341`, `cli.py:356`, `api.py:103`); non-loopback prints an exposure banner (`cli.py:406`) | **None in the default deployment.** In the exposed deployment: accepted, opt-in, and warned |
| 1 | Unauthenticated peer submits a job | Spoofing / Elevation | `POST /api/jobs` without a token | *Default:* no reachable socket. *Exposed:* `_authed` before body read, `server.py:339` | Low — this is the enforced boundary |
...
| 3 | Token read from the process table | Information disclosure | `ps` / Task Manager on any mesh host | None. `--token` is argv (on `serve` and every other subcommand's parser); `serve` also prints it to stdout (`cli.py:493`) | **Accepted.** Use `GPUMESH_TOKEN` on shared hosts |
...
| 7 | Malicious coordinator serves a hostile `_func` | Elevation | `POST /api/lease` response -> `serializer.py:217` | Token; `--safe-mode` on the worker (`worker.py:832`) | **Accepted.** Safe mode reduces it to script tasks, which still execute |
| 8 | Token holder steals credentials from `$HOME` | Info disclosure | Any task body | None; granted capability | **Accepted by design** |
| 9 | Token holder exhausts GPU / memory / disk | Denial of service | Any task body | Per-task timeout only (`--timeout`, default 240 s). `user_error` failures are final rather than retried 3x (`db.py:845`; set at `sandbox.py:156/165/171` and `worker.py:160`), removing an accidental amplifier — not a quota | **Accepted by design.** No quotas |
| 10 | Unauthenticated flood of the coordinator port | Denial of service | Connection or request flood | *Default:* no reachable socket. *Exposed:* 10 MB body cap (`server.py:121`); `ThreadingHTTPServer` with no connection cap and **no socket timeout on the coordinator handler** | **Accepted.** In an exposed deployment a LAN peer can still exhaust threads without a token. The claim port's `timeout = 15` fix (row 15) has no coordinator equivalent |
...
| 12 | Spoofed discovery beacon | Spoofing | UDP broadcast on 48900 | Field clamping (`discovery.py:254-271`); peers are printed (`server.py:347`), never auto-claimed | Low. Cosmetic pollution of the operator's console |
...
| 14 | Unauthenticated claim of an idle worker | Elevation | `POST /api/claim` with a guessed token | `hmac.compare_digest` (`claimer.py:478`, and `worker.py:1029` in the patched handler); single-claim latch (`claimer.py:489-494`); **per-IP rate limiting, same policy as the coordinator** (`claimer.py:217`, accounted in `_send`, `claimer.py:287-289`) | Low. The "no rate limiting on this port" gap is closed |
| 15 | Pre-auth JSON parsing on the claim port | Tampering / DoS | Body parsed before the token check — unavoidable, the token is *in* the body | Rate limit consulted first (`claimer.py:356`); 16 KB cap refused on the declared length before allocation (`claimer.py:227`, `claimer.py:407`); chunked and length-less refused 411 (`claimer.py:381`, `claimer.py:389`); drain bounded to 256 KB / 2 s (`claimer.py:236-237`) | Low. `json.loads` over ≤16 KB, for non-locked-out peers only. Still the only unauthenticated structured parse in the codebase |
| 15b | Slowloris on the claim port | Denial of service | Open connections declaring a body that never arrives; one thread pinned per connection forever | **Fixed.** `timeout = 15` on the handler (`claimer.py:246`), applied to the connection socket by `StreamRequestHandler.setup()` | Low. A generous 15 s for a sub-kilobyte claim |
| 16 | SSRF from a claimed worker | Elevation / Info disclosure | `coordinator_url` from the request body -> `find_reachable_coordinator` (called at `claimer.py:504`) | Worker token required first | Low. A token bypass on the claim port turns into SSRF *and* RCE |
| 17 | Token leaked into a log or traceback | Info disclosure | Handler exceptions print tracebacks (`server.py:329-333` for GET, `server.py:517-521` for POST) | Handler logging is suppressed (`server.py:283`, and `claimer.py:452`); the token is not in coordinator request bodies | Low. Would be a reportable bug — see `SECURITY.md`. Note the claim token *is* in a claim body, so a claim-handler traceback would be a real leak; none is printed today |
...
| 19 | Repudiation: who submitted this job? | Repudiation | One shared token, no identities | None | **Accepted by design.** Job events record worker joins/leaves (`db.record_event`, `db.py:464`), never submitter identity |
| 20 | Token file readable by other local users | Info disclosure | `~/.gpumesh/config.json` in plaintext | 0600 + `icacls`; **failures now warn** (`connection_manager.py:44`) and a group/other-readable file warns on load (`connection_manager.py:135`) | Low on POSIX. **Residual on Windows:** `st_mode` carries no ACL information, so a post-hoc ACL widening is not detected (`connection_manager.py:156`) |
...
3. **No isolation of tasks.** Same OS user, same `$HOME`, no namespaces, no
   ...
   (`worker.py:201`, `sandbox.py:107`) is for crash containment and cleanup.
...
5. **LAN exposure is opt-in, and opting in is accepted.** `gpumesh serve` and
   `GPUMesh.start_coordinator` bind loopback by default (`cli.py:341`,
   `api.py:103`). Once the operator passes `--host 0.0.0.0` or sets
   ...
   (`cli.py:406`) names the OS user and device that submitted code will run
   ...
   (`claimer.py:585`, hardened per §5.2), and `gpumesh setup` in LAN
...
6. **The token is a single shared secret with no rotation, expiry, or
   ...
   every worker (`worker.py:604-620`).
...
8. **No socket timeout on the coordinator handler.** Row 10. The claim port
   got `timeout = 15` (`claimer.py:246`); the coordinator's
...
- **Written:** 2026-08-15
- **Last reconciled against the code:** 2026-08-15
- **Against commit:** `96eee91` ("Refuse cross-version function tasks when no
  source fallback exists"), plus the uncommitted working tree of that date —
  which includes the loopback bind default, the claim-server hardening, the
  loopback rate-limit exemption, the `user_error` changes, the claim-server
  bind knob (`GPUMESH_CLAIM_HOST`), and the token-file permission warnings.
  None of those are in `96eee91` itself.
- **Version:** gpumesh 1.3.0

> [!IMPORTANT]
> **The line numbers go stale faster than the claims do.** Between the first
> reconciliation and the second, `claimer.py` grew ~90 lines at the top and
> `cli.py` grew by roughly a thousand, which moved almost every citation in
> this document without changing a single thing it asserts. They have been
> re-anchored, but treat them as a *finding aid*, not as evidence: the symbol
> name in the same sentence is the durable half of every citation, and the
> claim is what was verified. If a number does not land, search for the
> symbol.
