...
If you have a *question* rather than a change, such as "how do I" or "is this supposed
...
gpumesh is early-stage, at `Development Status :: 4 - Beta`. It works, it is
...
Python 3.9 through 3.14, and every one of those is a cell in the CI matrix, and
...
`dev` is deliberately small, holding `pytest`, `pytest-cov`, `build` and `twine`, because
...
| `server.py` | Coordinator HTTP API. Thin, since logic belongs in `db.py` |
...
| `mesh.py` | `@mesh`, a zero-config wrapper over `accelerate.py` |
...
the code already is, measured rather than chosen as an ideal, so it flags the
...
- 4-space indent, double quotes. Most lines stay under 80 characters and
  almost none exceed 100, and no hard limit is enforced
...
**The coverage floor is 80%**, and the test job runs
...
6. Sign off your commits with `git commit -s` (see [below](#sign-your-commits-dco))
7. Open the PR
...
short hyphenated description, such as `fix/worker-timeout-message`,
`docs/windows-setup`.
