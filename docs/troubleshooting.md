...
| **Another machine cannot connect at all** | The coordinator binds `127.0.0.1` by default. Restart it with `--host 0.0.0.0` (or `GPUMESH_HOST=0.0.0.0`). Check this **before** the firewall, because a loopback bind and a blocked firewall look identical from the other machine |
...
| `GPUMESH_HOST_IP` seems to be ignored | It takes an IP literal, not a hostname, and almost any typo is a syntactically valid hostname, so it is validated and a non-IP value is discarded with a `WARNING` line at startup. `gpumesh doctor` prints the address actually being advertised |
...
| `Cannot run this task on a Python X worker` | Same cause, no source to fall back to, because the function was defined in a REPL or heredoc. Put it in a `.py` file |
