...
WORKER_DELETE_AFTER = 300.0 # seconds before stale worker row is deleted
...
class Database:
    ...
    def heartbeat(self, worker_id: str, task_id: str | None = None,
        ...
        **ignored**: the liveness half of the heartbeat still lands, the lease
        ...
        with self._lock, self._conn:
            ...
            # Store latest GPU free memory for memory-aware scheduling
    ...
    def lease_task(self, worker_id: str):
        ...
        * Under straggler deprioritization, which overrides score order on
        ...
        Straggler tolerance: workers with avg_time > 2x the median
        avg_time are deprioritized — they get lighter tasks to reduce
        the impact of their slowness.

        Memory-aware scheduling: if a task's payload contains a
        ``gpu_memory_mb`` hint, prefer workers with enough free GPU memory.
        ...
        with self._lock, self._conn:
            ...
            # what the straggler rule actually means.
            ...
            # Filter tasks by their placement hints.
    ...
    def complete_task(self, task_id: str, worker_id: str, ok: bool,
        ...
        with self._lock, self._conn:
            ...
            # Update straggler stats — only on success
