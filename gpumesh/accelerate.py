...
Usage:
    ...
    # Pattern 4: Batch call
    results = train.map([
        {"lr": 0.01, "epochs": 100},
        {"lr": 0.05, "epochs": 200},
    ])

    # Pattern 5: Auto device placement
    @accelerate(mesh)
    def train_model(model, data):
        # model is automatically placed on best device
        return model(data)

    # Pattern 6: Import hook
...
class AcceleratedFunction:
    ...
    def map(self, params_list: list[dict]) -> list[dict]:
        """Distribute across all mesh devices.
...
def install(mesh: Any = None) -> None:
    """Install gpumesh as a transparent import hook.
