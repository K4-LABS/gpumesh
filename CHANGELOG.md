...
- **Straggler detection was silently disabled on two-worker meshes.** The
  ...
  from the median it is measured against, which is what the straggler rule
  actually means.
...
### Added
- **Process isolation for remote jobs** — Tasks run in separate subprocesses.
- **Benchmark scoring system** — `run_benchmark()` returns a 0-100 score per worker.
- **Crash diagnostics** — Structured error reporting on worker failures.
- **TTL-based worker expiry** — Stale workers automatically pruned.
- **Straggler deprioritization** — Slow workers get fewer jobs.
- **GPU memory tracking** — `get_gpu_memory_usage()` reports VRAM usage.
- **Memory-aware scheduling** — Workers selected by available VRAM.
- **Node join/leave events** — `GET /api/events` endpoint for real-time mesh changes.
- **Graceful fallback** — `@accelerate` falls back to local execution if mesh is unavailable.
- **Worker stats tracking** — `worker_stats` table records jobs, errors, latency per worker.
