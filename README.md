...
```console
$ gpumesh --version
gpumesh 3.2.0 (3.11.9, Windows)
```

**`'gpumesh' is not recognized` / `command not found`?** The package installed
correctly; its console script just landed in a directory that is not on your
`PATH`. This is common on Windows, and after any `pip install --user`. Every
command on this page works with `python -m gpumesh` in place of `gpumesh`:

```console
$ python -m gpumesh --version
```

To fix it rather than work around it, add the scripts directory to `PATH`.
This prints the one to add:
...
with the same scheduler, wire format and results. Get that working first;
...
Your function returns whatever it normally returns, whether a dict, an int, a list, a numpy array or a torch tensor, and you get that same object back:
...
Every command, covering the server, jobs and monitoring, plus the environment variables
...
Drop the `127.0.0.1:` prefix, or better, name a specific interface such as
...
The default, meaning no flag at all, is **loopback only**. Every row above is an
...
> The coordinator derives its hash with PBKDF2-HMAC-SHA256, using a random 16-byte
...
The usual suspects, covering bind vs firewall, mismatched tokens, cross-version tasks and
...
- [**CONTRIBUTING.md**](https://github.com/K4-LABS/gpumesh/blob/master/CONTRIBUTING.md)
  covers dev setup, how the pieces fit together, and what makes a change easy to review
- [**Good first issues**](https://github.com/K4-LABS/gpumesh/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22)
  lists scoped starting points that name the file to change
- [**Issue tracker**](https://github.com/K4-LABS/gpumesh/issues) for bugs, and
...
- Arguments and return values must be picklable. Anything tied to a live process cannot cross machines, including open files, sockets, locks, database handles and CUDA handles. Return plain data instead: numbers, arrays, tensors, DataFrames
...
- Single coordinator, so a single point of failure (use Tailscale for reliability)
